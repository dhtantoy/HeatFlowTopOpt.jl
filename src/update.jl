"""
    random_window!(A::Array, kernel::Array, ratio::Real, seed)
in-place generation of a random `0-1` `kernel` with seed and volume `vol`,
and upsample and store it to the size of `A` as a window.
"""
function random_window!(A::Array, kernel::Array, ratio::Real, seed) 
...
`A + w(B - A) -> A`.
"""
function post_interpolate!(A::Array, B::Array, w::Real)
    r = 1 - w
...
`A + W⊗(B - A) -> A`.
"""
function post_interpolate!(A::Array, B::Array, W::Array{T}) where T
    @turbo for i = eachindex(A)
        A[i] = B[i] + W[i] * (A[i] - B[i])
...
"""
function post_phi!(Φ::Array{T1}, Gτχ::Array{T2}, Φ_max::T1, Φ_min::T1, down::T2= typemin(T2), up::T2= typemax(T2)) where {T1, T2}
    Φ_max += one(T1)
    Φ_min -= one(T1)
...
"""
function rand_post_phi!(Φ::Array{T}, perm::Array{<:Integer}, Φ_max::T, n, seed) where {T}
...
"""
function iterateχ!(χ::Array{T}, order::SzVector{Int}, Φ::AbstractArray{T}, M::Real) where {T}
    fill!(χ, zero(T))
    ix = order.data
...
"""
function get_sorted_idx!(order_idx::SzVector, cache_val::SzVector, cache_idx::SzVector, χ, weight)
    resize!(cache_val, 0)
    resize!(cache_idx, 0)
    T = eltype(χ)
...
"""
function phi_to_prob!(P::Array, Φ::Array{T}, Φ_max::T) where {T}
...
"""
function prob_to_weight!(weight::Array, P::Array, cache_idx::Array{T}) where {T <: Integer}
    a = one(T):length(P)
    wv = pweights(P)
...
"""
function nonsym_correct!(χ::Array, sort_idx_dec::SzVector, sort_idx_inc::SzVector, θ::Real)
